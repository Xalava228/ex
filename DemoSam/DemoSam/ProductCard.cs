    ...
    public partial class ProductCard : UserControl
    {
        ...
        public void LoadProduct(DataRow row)
        {
            ...
            if (row["Photo"] != DBNull.Value && row["Photo"] != null)
            {
                try
                {
                    string path = $"..\\..\\image\\{row["Photo"].ToString()}";
                    if (File.Exists(path))
                    {
                        var old = pictureBoxPhoto.Image;
                        pictureBoxPhoto.Image = LoadImageNoLock(path);
                        old?.Dispose();
                    }
                    else
                    {
        ...
        private void ProductCard_Click(object sender, EventArgs e)
        {
            ...
            {
                if (DetailProduct.TryActivateOpenedEditor())
                {
                    MessageBox.Show("Окно редактирования товара уже открыто.");
                    return;
                }

                using (DetailProduct detailProduct = new DetailProduct(_row))
                {
